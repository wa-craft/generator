...
import * as wrapper from './wrapper/mod.ts';

/** */
export default class Backend extends AbstractGenerator {
	data: any = {};
	pluginPath: string = '';
	templatePath: string = '';
	...
	async paths(): Promise<void> {
		...
		for (const i in preRoutes) {
			...
			for (const j in tmpRoutes) {
				if (
					tmpRoutes[j] !== undefined &&
					newRoute.className === tmpRoutes[j].className &&
					newRoute.path === tmpRoutes[j].path
				) {
	...
	async models(): Promise<void> {
		...
		for (const schemaKey of Object.keys(schemas)) {
			...
			model = new wrapper.Model(schemas[schemaKey] ?? {});
